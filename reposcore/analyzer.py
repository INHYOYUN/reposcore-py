...
class RepoAnalyzer:
    ...
    def calculate_scores(self) -> Dict:
        ...
        def calculate_valid_pr(p_fb, p_d):
            return p_fb + min(p_d, 3 * max(p_fb, 1))

        def calculate_valid_issue(i_fb, i_d, p_valid):
            return min(i_fb + i_d, 4 * p_valid)

        def distribute_pr_score(p_fb, p_valid):
            return min(p_fb, p_valid), max(p_valid - p_fb, 0)

        def distribute_issue_score(i_fb, i_valid):
            return min(i_fb, i_valid), max(i_valid - i_fb, 0)

        scores = {}
        total_score_sum = 0

        for user, a in self.participants.items():
            p_fb = a.get('p_enhancement', 0) + a.get('p_bug', 0)
            p_d  = a.get('p_documentation', 0)
            i_fb = a.get('i_enhancement', 0) + a.get('i_bug', 0)
            i_d  = a.get('i_documentation', 0)

            p_valid = calculate_valid_pr(p_fb, p_d)
            i_valid = calculate_valid_issue(i_fb, i_d, p_valid)

            pr_fb, pr_doc       = distribute_pr_score(p_fb, p_valid)
            issue_fb, issue_doc = distribute_issue_score(i_fb, i_valid)

            total = (
                self.score['feat_bug_pr'] * pr_fb +
                self.score['doc_pr'] * pr_doc +
                self.score['feat_bug_is'] * issue_fb +
                self.score['doc_is'] * issue_doc
            )

            scores[user] = {
                "feat/bug PR":     self.score['feat_bug_pr'] * pr_fb,
                "document PR":     self.score['doc_pr'] * pr_doc,
                "feat/bug issue":  self.score['feat_bug_is'] * issue_fb,
                "document issue":  self.score['doc_is'] * issue_doc,
                "total":           total
            }
        ...
        return dict(
            sorted(scores.items(), key=lambda x: x[1]["total"], reverse=True)
        )
    ...
    def calculate_averages(self, scores):
        ...
        if not scores:
            return dict.fromkeys([
                "feat/bug PR", "document PR",
                "feat/bug issue", "document issue",
                "total", "rate"
            ], 0)

        avg = {k: 0 for k in list(scores.values())[0].keys()}
        ...
        n = len(scores)
        return {k: round(v / n, 1) for k, v in avg.items()}

    def generate_table(self, scores: Dict, save_path) -> None:
        # 나머지 함수들은 원본 그대로 두겠습니다
